...
pub fn generate_features(
    ...
    // Optional: connect all attributes to all labels (zero-freq state features)
    if connect_all_attrs {
        for aid in 0..num_attrs as i32 {
    ...
    feature_map
        ...
        .map(|((ftype, src, dst), freq)| Feature { ftype, src, dst, freq })
        .collect()
}
...
pub fn init_references(
    ...
    let mut label_refs: Vec<FeatureRefs> = (0..num_labels).map(|_| FeatureRefs::default()).collect();
...
mod tests {
    use super::*;
    use crate::types::{Attribute, Item, Instance};

    fn make_instance(items: Vec<Vec<(i32, f64)>>, labels: Vec<i32>) -> Instance {
        Instance {
            items: items.into_iter().map(|attrs| Item {
                contents: attrs.into_iter().map(|(aid, val)| Attribute { aid, value: val }).collect(),
            }).collect(),
            labels,
            weight: 1.0,
            group: 0,
    ...
    fn test_generate_basic() {
        ...
        assert_eq!((features[0].ftype, features[0].src, features[0].dst), (0, 0, 0));
        assert_eq!((features[1].ftype, features[1].src, features[1].dst), (0, 1, 0));
        assert_eq!((features[2].ftype, features[2].src, features[2].dst), (0, 2, 1));
        assert_eq!((features[3].ftype, features[3].src, features[3].dst), (1, 0, 1));
    }
    ...
    fn test_init_references() {
        let features = vec![
            ...
            Feature { ftype: FT_TRANS, src: 0, dst: 1, freq: 1.0 },
        ];
